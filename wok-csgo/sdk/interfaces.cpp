...
namespace interfaces {
	void init() {
		INTERFACE_VAR(client_dll, i_base_client_dll, "client.dll", "VClient018")
		INTERFACE_VAR(engine, i_engine_client, "engine.dll", "VEngineClient014")
		INTERFACE_VAR(entity_list, i_client_entity_list, "client.dll", "VClientEntityList003")
		...
		INTERFACE_VAR(model_info, i_model_info, "engine.dll", "VModelInfoClient004")
		INTERFACE_VAR(surface, i_surface, "vguimatsurface.dll", "VGUI_Surface031")
		...
		INTERFACE_VAR(prediction, i_prediction, "client.dll", "VClientPrediction001")
		INTERFACE_VAR(game_movement, i_game_movement, "client.dll", "GameMovement001")
		INTERFACE_VAR(debug_overlay, i_debug_overlay, "engine.dll", "VDebugOverlay004")
		INTERFACE_VAR(cvar_system, i_cvar_system, "vstdlib.dll", "VEngineCvar007")
		INTERFACE_VAR(event_manager, i_game_event_manager, "engine.dll", "GAMEEVENTSMANAGER002")
		INTERFACE_VAR(trace_system, i_engine_trace, "engine.dll", "EngineTraceClient004")
		INTERFACE_VAR(surface_data, i_surface_data, "vphysics.dll", "VPhysicsSurfaceProps001")
		INTERFACE_VAR(input_system, i_input_system, "inputsystem.dll", "InputSystemVersion001")
		INTERFACE_VAR(leaf_system, i_client_leaf_system, "client.dll", "ClientLeafSystem002")

		INTERFACE_SIG(game_rules, c_game_rules*, "client.dll", "A1 ? ? ? ? 85 C0 0F 84 ? ? ? ? 80 B8 ? ? ? ? ? 74 7A", 0x1)

		INTERFACE_VAR(model_cache, i_mdl_cache, "datacache.dll", "MDLCache004")
		INTERFACE_VAR(model_render, i_model_render, "engine.dll", "VEngineModel016")
		INTERFACE_VAR(material_system, i_material_system, "materialsystem.dll", "VMaterialSystem080")
		...
		INTERFACE_SIG(player_resource, c_cs_player_resource*, "client.dll", "8B 3D ? ? ? ? 85 FF 0F 84 ? ? ? ? 81 C7", 0x2)

		INTERFACE_VAR(localize, i_localize, "localize.dll", "Localize_001")
		INTERFACE_VAR(panel, i_panel, "vgui2.dll", "VGUI_Panel009")

		uintptr_t** shader_device = nullptr;
		INTERFACE_VAR(shader_device, uintptr_t*, "shaderapidx9.dll", "ShaderDevice001")
